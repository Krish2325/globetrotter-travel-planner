...
const PIE=[{name:"Mobile",value:58,color:"#F2683A"},{name:"Desktop",value:32,color:"#FB8A4C"},{name:"Tablet",value:10,color:"#FFC9A3"}];
          ...
          <div style={{padding:"1.5rem"}}>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={MONTHLY}>
                <defs>
                  <linearGradient id="s1" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#F2683A" stopOpacity={0.3}/><stop offset="95%" stopColor="#F2683A" stopOpacity={0}/></linearGradient>
                ...
                <Area type="monotone" dataKey="searches" stroke="#F2683A" strokeWidth={2} fill="url(#s1)" name="Searches"/>
        ...
        <motion.div style={card} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.2}}>
          <div style={{padding:"1.5rem"}}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={SEASONAL}><CartesianGrid strokeDasharray="3 3" stroke="rgba(214,122,74,0.1)"/>
                <XAxis dataKey="s" tick={{fontSize:11,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fontSize:11,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/>
                <Tooltip contentStyle={{background:"#fff",border:"1px solid #EFEDE7",borderRadius:12,fontSize:12}}/>
                <Legend iconSize={8} wrapperStyle={{fontSize:12}}/>
                <Bar dataKey="beach" fill="#F2683A" radius={[4,4,0,0]} name="Beach"/>
                <Bar dataKey="mountain" fill="#FFC9A3" radius={[4,4,0,0]} name="Mountain"/>
                <Bar dataKey="cultural" fill="#FFA76B" radius={[4,4,0,0]} name="Cultural"/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
              ...
              {SEARCHED.map((d,i)=>(
                <div key={d.dest} style={{display:"flex",alignItems:"center",gap:"0.75rem"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{height:5,borderRadius:10,background:"#EFEDE7",overflow:"hidden"}}><div style={{height:"100%",borderRadius:10,background:"linear-gradient(135deg,#F2683A,#FB8A4C)",width:((d.count/SEARCHED[0].count)*100)+"%",transition:"width 0.6s ease"}}/></div>
                  </div>
                </div>
              ))}
            </div>
