...
const PIE=[{name:"Mobile",value:58,color:"#4F46E5"},{name:"Desktop",value:32,color:"#6366F1"},{name:"Tablet",value:10,color:"#A5B4FC"}];
          ...
          <div style={{padding:"1.5rem"}}>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={MONTHLY}>
                <defs>
                  <linearGradient id="s1" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#4F46E5" stopOpacity={0.3}/><stop offset="95%" stopColor="#4F46E5" stopOpacity={0}/></linearGradient>
                ...
                <Area type="monotone" dataKey="searches" stroke="#4F46E5" strokeWidth={2} fill="url(#s1)" name="Searches"/>
        ...
        <motion.div style={card} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.2}}>
          <div style={{padding:"1.5rem"}}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={SEASONAL}><CartesianGrid strokeDasharray="3 3" stroke="rgba(99, 102, 241,0.1)"/>
                <XAxis dataKey="s" tick={{fontSize:11,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fontSize:11,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/>
                <Tooltip contentStyle={{background:"#fff",border:"1px solid #ECECF7",borderRadius:12,fontSize:12}}/>
                <Legend iconSize={8} wrapperStyle={{fontSize:12}}/>
                <Bar dataKey="beach" fill="#4F46E5" radius={[4,4,0,0]} name="Beach"/>
                <Bar dataKey="mountain" fill="#A5B4FC" radius={[4,4,0,0]} name="Mountain"/>
              ...
              {SEARCHED.map((d,i)=>(
                <div key={d.dest} style={{display:"flex",alignItems:"center",gap:"0.75rem"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{height:5,borderRadius:10,background:"#ECECF7",overflow:"hidden"}}><div style={{height:"100%",borderRadius:10,background:"linear-gradient(135deg,#4F46E5,#6366F1)",width:((d.count/SEARCHED[0].count)*100)+"%",transition:"width 0.6s ease"}}/></div>
                  </div>
                </div>
              ))}
            </div>
