  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
      ...
      <div style={{display:"flex",gap:"0.5rem",flexWrap:"wrap"}}>
        {CATS.map(c=>(
          <button key={c} onClick={()=>setCat(c)} className="btn" style={{padding:"0.4rem 0.875rem",borderRadius:20,fontSize:"0.8125rem",background:cat===c?"linear-gradient(135deg,#4F46E5,#6366F1)":"rgba(255,255,255,0.7)",color:cat===c?"#fff":"#6B7280",border:"1px solid rgba(99, 102, 241,0.2)"}}>
            {c}
          </button>
        ))}
      </div>
          ...
          <motion.div key={a.id} style={card} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.06}} whileHover={{y:-3}}>
            <div style={{padding:"1.25rem"}}>
              ...
              <div style={{display:"flex",gap:"1rem"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4,fontSize:"0.875rem",fontWeight:700,color:"#4F46E5"}}>{a.cost}</div>
              </div>
            </div>
          </motion.div>
        ))}
