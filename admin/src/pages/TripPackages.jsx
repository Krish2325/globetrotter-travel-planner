    ...
    <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
      ...
      {loading ? (
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:"1.25rem"}}>
          {sorted.map((p,i)=>(
            <motion.div key={p.id} style={card} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.07}} whileHover={{y:-5,boxShadow:"0 20px 50px rgba(242,104,58,0.14)"}}>
              ...
              <div style={{padding:"1.25rem"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <p style={{fontSize:"1.375rem",fontWeight:800,fontFamily:"Poppins,sans-serif",color:"#F2683A"}}>{p.basePrice ? `Rs. ${p.basePrice}` : "Contact for price"}</p>
