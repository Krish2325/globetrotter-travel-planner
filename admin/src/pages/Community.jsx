      ...
      <motion.div style={card} initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.1}}>
        <div style={{padding:"1.5rem"}}>
          <div style={{display:"flex",gap:"0.75rem",marginBottom:"1.25rem",flexWrap:"wrap"}}>
            ...
            <div style={{display:"flex",gap:"0.5rem"}}>
              {["All","active","flagged"].map(f=>(
                <button key={f} onClick={()=>setFilter(f)} className="btn" style={{padding:"0.5rem 0.875rem",background:filter===f?"linear-gradient(135deg,#4F46E5,#6366F1)":"rgba(255,255,255,0.7)",color:filter===f?"#fff":"#6B7280",border:"1px solid rgba(99, 102, 241,0.2)",fontSize:"0.8125rem",borderRadius:12}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"0.75rem"}}>
            {filtered.map(p=>(
              <motion.div key={p.id} whileHover={{x:3}} style={{display:"flex",alignItems:"center",gap:"1rem",padding:"1rem 1.25rem",borderRadius:16,background:p.status==="flagged"?"rgba(239,68,68,0.05)":"rgba(99, 102, 241,0.04)",border:"1px solid "+(p.status==="flagged"?"rgba(239,68,68,0.2)":"rgba(99, 102, 241,0.12)")}}>
