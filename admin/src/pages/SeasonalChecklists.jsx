...
export default function SeasonalChecklists() {
  ...
  const [newItems, setNewItems] = useState({});   // { [cat]: string }
  ...
  const addItem = (cat) => {
    ...
    setTemplates(templates.map(t =>
      t.id === sel.id
        ? { ...t, categories: { ...t.categories, [cat]: [...t.categories[cat], val] } }
        : t
    ));
  ...
  const deleteItem = (cat, idx) => {
    setTemplates(templates.map(t =>
      t.id === sel.id
        ? { ...t, categories: { ...t.categories, [cat]: t.categories[cat].filter((_, i) => i !== idx) } }
        : t
    ));
  };
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
      ...
      <div className="rg-checklist">
        <div style={{display:"flex",flexDirection:"column",gap:"0.625rem"}}>
          {templates.map(t=>(
            <motion.div key={t.id} onClick={()=>{ setSelId(t.id); setOpenCat(null); }} whileHover={{x:4}} style={{...card,padding:"1rem",cursor:"pointer",borderRadius:16,background:selId===t.id?"rgba(79, 70, 229,0.1)":"rgba(255,255,255,0.65)",border:"1px solid "+(selId===t.id?"rgba(79, 70, 229,0.4)":"rgba(99, 102, 241,0.18)")}}>
        ...
        <div style={card}>
          <div style={{padding:"1.5rem"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.25rem"}}>
              <div><h2 className="section-title">{sel.season} - {sel.type}</h2><p style={{fontSize:"0.8rem",color:"#9CA3AF",marginTop:2}}>Click a category to expand & add items</p></div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:"0.75rem"}}>
              {Object.entries(sel.categories).map(([cat,items])=>(
                <div key={cat} style={{borderRadius:16,border:"1px solid rgba(99, 102, 241,0.15)",overflow:"hidden"}}>
                  <button onClick={()=>setOpenCat(openCat===cat?null:cat)}
                    style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0.875rem 1rem",background:"rgba(99, 102, 241,0.06)",border:"none",cursor:"pointer",textAlign:"left"}}>
                    ...
                    {openCat===cat&&(
                      <motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}} style={{overflow:"hidden"}}>
                        <div style={{padding:"0.75rem 1rem",display:"flex",flexDirection:"column",gap:"0.5rem"}}>
                          {items.map((item,i)=>(
                            <div key={i} style={{display:"flex",alignItems:"center",gap:"0.75rem",padding:"0.5rem 0.75rem",borderRadius:12,background:i%2===0?"rgba(99, 102, 241,0.04)":"transparent"}}>
                              <CheckSquare size={13} color="#A5B4FC"/>
                          ...
                          <div style={{display:"flex",gap:"0.5rem",marginTop:"0.5rem"}}>
                            ...
                            />
                            <button onClick={()=>addItem(cat)} className="btn btn-primary" style={{padding:"0.5rem 0.875rem",fontSize:"0.8rem"}}><Plus size={13}/>Add</button>
                          </div>
                        </div>
                      </motion.div>
                    )}
