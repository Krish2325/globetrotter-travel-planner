...
const NOTIFS = [
  {id:1,type:"user",icon:User,color:"#4F46E5",title:"New Admin Registered",desc:"Areen Sharma just created an admin account.",time:"2 min ago",read:false},
  ...
  {id:5,type:"user",icon:User,color:"#4F46E5",title:"New User Milestone",desc:"Globetrotter reached 1,000 registered users!",time:"5 hrs ago",read:true},
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
      <motion.div initial={{opacity:0,y:-12}} animate={{opacity:1,y:0}} style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:"1rem"}}>
        <div style={{display:"flex",alignItems:"center",gap:"0.75rem"}}>
          ...
          {unread>0&&<span style={{background:"linear-gradient(135deg,#4F46E5,#6366F1)",color:"#fff",borderRadius:20,padding:"0.2rem 0.6rem",fontSize:"0.75rem",fontWeight:700}}>{unread} new</span>}
        </div>
      ...
      <div style={{display:"flex",gap:"0.5rem"}}>
        {["All","Unread","Read"].map(f=>(
          <button key={f} onClick={()=>setFilter(f)} className="btn" style={{padding:"0.45rem 1rem",borderRadius:20,fontSize:"0.8125rem",background:filter===f?"linear-gradient(135deg,#4F46E5,#6366F1)":"rgba(255,255,255,0.7)",color:filter===f?"#fff":"#6B7280",border:"1px solid rgba(99, 102, 241,0.2)"}}>
            {f}
          </button>
        ))}
      </div>
          ...
          <AnimatePresence>
            ...
            {filtered.map(n=>(
              <motion.div key={n.id} layout initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}} exit={{opacity:0,x:16}}
                ...
                style={{display:"flex",alignItems:"flex-start",gap:"1rem",padding:"1rem",borderRadius:16,marginBottom:"0.375rem",cursor:"pointer",background:n.read?"transparent":"rgba(79, 70, 229,0.04)",border:"1px solid "+(n.read?"transparent":"rgba(79, 70, 229,0.1)"),transition:"all 0.2s"}}>
