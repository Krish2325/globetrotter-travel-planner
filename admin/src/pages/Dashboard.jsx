...
const PIE_DATA=[{name:"Beach",value:35,color:"#F2683A"},{name:"Mountain",value:28,color:"#FB8A4C"},{name:"Cultural",value:20,color:"#FFC9A3"},{name:"Adventure",value:17,color:"#FFA76B"}];
...
export default function Dashboard() {
  ...
  const STATS=[
    {label:"Total Users",value:stats.totalUsers,icon:Users,color:"#F2683A"},
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
          ...
          <div style={{padding:"1.5rem"}}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={AREA_DATA}>
                <defs>
                  <linearGradient id="gu" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#F2683A" stopOpacity={0.25}/><stop offset="95%" stopColor="#F2683A" stopOpacity={0}/></linearGradient>
                ...
                <Area type="monotone" dataKey="users" stroke="#F2683A" strokeWidth={2} fill="url(#gu)" name="Users"/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem"}}>
        <motion.div style={card} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.3}}>
          <div style={{padding:"1.5rem"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:"1.25rem"}}><Star size={16} color="#F2683A"/><p className="section-title">Recent Trips (Live)</p></div>
            <div style={{display:"flex",flexDirection:"column",gap:"0.625rem"}}>
              ...
              {recentTrips.map((t,i)=>(
                <div key={t.id||i} style={{display:"flex",alignItems:"center",gap:"0.75rem",padding:"0.625rem 0.75rem",borderRadius:14,background:i%2===0?"rgba(214,122,74,0.05)":"transparent"}}>
        ...
        <motion.div style={card} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.35}}>
          <div style={{padding:"1.5rem"}}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={BAR_DATA}><CartesianGrid strokeDasharray="3 3" stroke="rgba(214,122,74,0.1)"/>
                <XAxis dataKey="s" tick={{fontSize:11,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fontSize:11,fill:"#9CA3AF"}} axisLine={false} tickLine={false}/>
                <Tooltip contentStyle={{background:"#fff",border:"1px solid #EFEDE7",borderRadius:12,fontSize:12}}/>
                <Legend iconSize={8} wrapperStyle={{fontSize:12}}/>
                <Bar dataKey="domestic" fill="#F2683A" radius={[6,6,0,0]} name="Domestic"/>
                <Bar dataKey="intl" fill="#FFC9A3" radius={[6,6,0,0]} name="International"/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
