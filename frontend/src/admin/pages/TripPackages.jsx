  ...
  return (
    <motion.div className="space-y-6" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {PKGS.map(p => <motion.div key={p.id} whileHover={{ y:-4, boxShadow:'0 16px 48px rgba(79, 70, 229,0.12)' }} className="rounded-3xl overflow-hidden" style={GLASS}><div className="relative h-44 overflow-hidden"><img src={p.img} alt={p.name} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" /><div className="absolute bottom-3 left-3 right-3 flex items-center justify-between"><span className="badge badge-teal">{p.season}</span><div className="flex items-center gap-1 text-white text-xs font-bold"><Star className="w-3 h-3" style={{ fill:'#FBBF24', color:'#FBBF24' }} />{p.rating}</div></div></div><div className="p-4"><h3 className="font-bold font-display text-sm" style={{ color:'#1F2937' }}>{p.name}</h3><div className="flex items-center justify-between mt-2"><span className="text-base font-bold font-display" style={{ color:'#4F46E5' }}>{p.price}</span><span className="text-xs" style={{ color:'#6B7280' }}>{p.duration}</span></div><button className="btn-secondary w-full mt-3 justify-center py-2 text-xs flex items-center gap-1.5"><Edit2 className="w-3 h-3" />Edit Package</button></div></motion.div>)}
      </div>
    </motion.div>
  );
}