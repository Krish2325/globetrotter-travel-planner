...
export default function ManageActivities() {
  ...
  return (
    <motion.div className="space-y-6" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
      ...
      <div className="flex gap-2 flex-wrap">{CATS.map(c => <button key={c} onClick={() => setCat(c)} className="px-4 py-2 rounded-2xl text-sm font-semibold transition-all" style={{ background: cat === c ? 'linear-gradient(135deg,#4F46E5,#6366F1)' : 'rgba(255,255,255,0.6)', color: cat === c ? '#fff' : '#6B7280', border:'1px solid rgba(99, 102, 241,0.2)' }}>{c}</button>)}</div>
      <div className="rounded-3xl p-6" style={GLASS}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(a => <motion.div key={a.id} whileHover={{ y:-2 }} className="rounded-2xl p-4" style={{ background:'rgba(99, 102, 241,0.06)', border:'1px solid rgba(99, 102, 241,0.15)' }}><div className="flex items-start justify-between mb-2"><span className="badge badge-teal">{a.category}</span><button className="btn-icon w-7 h-7 rounded-xl"><Edit2 className="w-3 h-3" /></button></div><h3 className="font-bold text-sm" style={{ color:'#1F2937' }}>{a.name}</h3>{a.durationMin && <p className="text-xs mt-1" style={{ color:'#6B7280' }}>{a.durationMin} min</p>}{a.costEstimate && <p className="text-sm font-bold mt-1" style={{ color:'#4F46E5' }}>Rs. {a.costEstimate}</p>}</motion.div>)}
