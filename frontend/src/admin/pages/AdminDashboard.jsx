...
const PIE_DATA = [{name:'Beach',value:35,color:'#4F46E5'},{name:'Mountain',value:28,color:'#A5B4FC'},{name:'Culture',value:22,color:'#6366F1'},{name:'Adventure',value:15,color:'#ECECF7'}];
...
export default function AdminDashboard() {
  ...
  const cards = [
    { icon:Map,         label:'Total Trips',      value:counts.trips,      trend:12,  color:'#4F46E5' },
    ...
    <motion.div className="space-y-7" initial="hidden" animate="show" variants={stagger}>
        ...
        <motion.div variants={fadeUp} className="lg:col-span-2 rounded-3xl p-6" style={GLASS}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={MONTHLY}>
              ...
              <Area type="monotone" dataKey="trips" stroke="#4F46E5" strokeWidth={2} fill="url(#g1)" name="Trips"/>
      ...
      <motion.div variants={stagger} className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <motion.div variants={fadeUp} className="rounded-3xl p-6" style={GLASS}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={CITIES} layout="vertical">
              ...
              <Tooltip contentStyle={{ background:'#fff', border:'1px solid #ECECF7', borderRadius:12, fontSize:12 }}/>
              <Bar dataKey="v" fill="#4F46E5" radius={[0,6,6,0]} name="Searches"/>
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        <motion.div variants={fadeUp} className="rounded-3xl p-6" style={GLASS}>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/trips" className="text-xs font-semibold inline-flex items-center gap-1" style={{ color:'#4F46E5' }}>View all <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <div className="space-y-3">
            {(recent.length ? recent : [{id:1,title:'Goa Getaway',status:'PLANNING'},{id:2,title:'Swiss Alps',status:'CONFIRMED'},{id:3,title:'Bali Dreams',status:'COMPLETED'}]).map((t,i) => (
              <div key={t.id} className="flex items-center gap-3 p-3 rounded-2xl" style={{ background:'rgba(99, 102, 241,0.06)', border:'1px solid rgba(99, 102, 241,0.12)' }}>
      ...
      <motion.div variants={fadeUp} className="rounded-3xl p-6" style={GLASS}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[{to:'/admin/trips',icon:Map,label:'Add Trip',color:'#4F46E5',bg:'rgba(79, 70, 229,0.1)'},{to:'/admin/cities',icon:Globe,label:'Add City',color:'#0369A1',bg:'rgba(3,105,161,0.1)'},{to:'/admin/activities',icon:Activity,label:'Add Activity',color:'#D97706',bg:'rgba(217,119,6,0.1)'},{to:'/admin/checklists',icon:PackageCheck,label:'New Checklist',color:'#7C3AED',bg:'rgba(124,58,237,0.1)'}].map(({ to, icon:Icon, label, color, bg }) => (
            <Link key={to} to={to}>
