...
const PIE_DATA = [{name:'Beach',value:35,color:'#F2683A'},{name:'Mountain',value:28,color:'#FFC9A3'},{name:'Culture',value:22,color:'#FB8A4C'},{name:'Adventure',value:15,color:'#EFEDE7'}];
...
export default function AdminDashboard() {
  ...
  const cards = [
    { icon:Map,         label:'Total Trips',      value:counts.trips,      trend:12,  color:'#F2683A' },
    ...
    <motion.div className="space-y-7" initial="hidden" animate="show" variants={stagger}>
        ...
        <motion.div variants={fadeUp} className="lg:col-span-2 rounded-3xl p-6" style={GLASS}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={MONTHLY}>
              <defs>
                <linearGradient id="g1" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#F2683A" stopOpacity={0.3}/><stop offset="95%" stopColor="#F2683A" stopOpacity={0}/></linearGradient>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(214,122,74,0.1)"/>
              ...
              <Area type="monotone" dataKey="trips" stroke="#F2683A" strokeWidth={2} fill="url(#g1)" name="Trips"/>
      ...
      <motion.div variants={stagger} className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <motion.div variants={fadeUp} className="rounded-3xl p-6" style={GLASS}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={CITIES} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(214,122,74,0.1)" horizontal={false}/>
              <XAxis type="number" tick={{ fontSize:10, fill:'#9CA3AF' }} axisLine={false} tickLine={false}/>
              <YAxis type="category" dataKey="city" tick={{ fontSize:11, fill:'#6B7280' }} axisLine={false} tickLine={false} width={50}/>
              <Tooltip contentStyle={{ background:'#fff', border:'1px solid #EFEDE7', borderRadius:12, fontSize:12 }}/>
              <Bar dataKey="v" fill="#F2683A" radius={[0,6,6,0]} name="Searches"/>
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        <motion.div variants={fadeUp} className="rounded-3xl p-6" style={GLASS}>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/trips" className="text-xs font-semibold inline-flex items-center gap-1" style={{ color:'#F2683A' }}>View all <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <div className="space-y-3">
            {(recent.length ? recent : [{id:1,title:'Goa Getaway',status:'PLANNING'},{id:2,title:'Swiss Alps',status:'CONFIRMED'},{id:3,title:'Bali Dreams',status:'COMPLETED'}]).map((t,i) => (
              <div key={t.id} className="flex items-center gap-3 p-3 rounded-2xl" style={{ background:'rgba(214,122,74,0.06)', border:'1px solid rgba(214,122,74,0.12)' }}>
      ...
      <motion.div variants={fadeUp} className="rounded-3xl p-6" style={GLASS}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[{to:'/admin/trips',icon:Map,label:'Add Trip',color:'#F2683A',bg:'rgba(242,104,58,0.1)'},{to:'/admin/cities',icon:Globe,label:'Add City',color:'#0369A1',bg:'rgba(3,105,161,0.1)'},{to:'/admin/activities',icon:Activity,label:'Add Activity',color:'#D97706',bg:'rgba(217,119,6,0.1)'},{to:'/admin/checklists',icon:PackageCheck,label:'New Checklist',color:'#7C3AED',bg:'rgba(124,58,237,0.1)'}].map(({ to, icon:Icon, label, color, bg }) => (
            <Link key={to} to={to}>
