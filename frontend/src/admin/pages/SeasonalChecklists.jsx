    ...
    <motion.div className="space-y-6" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="space-y-3">
          {lists.map(l => <motion.div key={l.id} whileHover={{ x:4 }} onClick={() => setSel(l)} className="rounded-2xl p-4 cursor-pointer" style={{ background: sel && sel.id === l.id ? 'rgba(242,104,58,0.1)' : 'rgba(255,255,255,0.6)', border: '1px solid ' + (sel && sel.id === l.id ? 'rgba(242,104,58,0.4)' : 'rgba(214,122,74,0.18)'), backdropFilter:'blur(12px)' }}><div className="flex items-center justify-between"><div><p className="font-bold text-sm" style={{ color:'#1F2937' }}>{l.season} - {l.tripType}</p><p className="text-xs mt-0.5" style={{ color:'#9CA3AF' }}>{l.items.length} items</p></div><span className="badge badge-teal">{l.season}</span></div></motion.div>)}
        </div>
        {sel ? (
