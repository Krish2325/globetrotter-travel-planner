    ...
    <motion.div className="space-y-6" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
      ...
      <div className="rounded-3xl p-6" style={GLASS}>
        <div className="flex gap-3 mb-5">
          ...
          <button className="btn-ghost border rounded-2xl px-4 flex items-center gap-2 text-sm" style={{ borderColor:'rgba(214,122,74,0.2)', background:'rgba(255,255,255,0.6)' }}><Filter className="w-4 h-4" />Filter</button>
        </div>
        {loading ? <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="skeleton h-16" />)}</div> : (
          <div className="space-y-2">
            ...
            {filtered.map((t, i) => (
              <div key={t.id} className="flex items-center gap-4 p-4 rounded-2xl" style={{ background:'rgba(214,122,74,0.05)', border:'1px solid rgba(214,122,74,0.1)' }}>
