...
exports.getShareBySlug = async (req, res, next) => {
  try {
...
exports.copyTrip = async (req, res, next) => {
  try {
    ...
    const newTripId = await prisma.$transaction(async (tx) => {
      ...
      await tx.tripCopy.create({ data: { originalTripId: original.id, copiedByUserId: req.user.id, newTripId: newTrip.id } });
      return newTrip.id;
    });

    res.status(201).json({ message: 'Trip copied successfully', tripId: newTripId });
  } catch (err) { handleError(err, res, next); }
};
