...
// Require at least one letter and one number so passwords aren't purely
// numeric or trivially short-but-simple strings.
...
exports.adminSignup = async (req, res) => {
  try {
    ...
    // Admin registration is closed unless an operator has configured a secret
    // invite code — otherwise anyone who finds the endpoint could grant
    // themselves ADMIN access.
    if (!process.env.ADMIN_SIGNUP_SECRET)
      return res.status(503).json({ error: 'Admin registration is currently disabled' });
    if (inviteCode !== process.env.ADMIN_SIGNUP_SECRET)
