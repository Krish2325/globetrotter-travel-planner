...
exports.adminGetAllTrips = async (req, res, next) => {
  try {
    ...
    res.json(toDisplay({ trips, total, page: parseInt(page), limit: parseInt(limit) }));
  ...
  try {
    ...
    const trip = await prisma.trip.create({
      data: {
        ...
        packageType,
        basePrice: toMinorUnits(basePrice),
        bestSeason, images,
    ...
    res.status(201).json(toDisplay(trip));
  } catch (err) { handleError(err, res, next); }
};
...
exports.adminUpdateTrip = async (req, res, next) => {
  try {
    ...
    if (req.body.basePrice !== undefined) data.basePrice = toMinorUnits(req.body.basePrice);
...
exports.updateTrip = async (req, res, next) => {
  try {
    ...
    const data = {};
    const fields = ['title','description','startingLocation','destination','packageType','bestSeason','images','coverImage','status'];
    const floats = ['rating']; const ints = ['durationDays','views','popularity','maxPeople'];
